    ...
    methods: {
        ...
        animateInImageRow(entries) {
            entries.forEach(({ target, isIntersecting, intersectionRatio}) => {
                if (isIntersecting) {
                    target.classList.add('md:transform-none', 'opacity-100');
...
<template>
    <div class="container lg:w-3/4 mx-auto flex justify-center flex-wrap relative mb-10">
        ...
        <div ref="rows" v-for="row in rows" class="opacity-0 transition-opacity md:transition-[opacity,transform] md:translate-y-[50px] md:duration-1000 md:ease-in w-full flex flex-wrap md:flex-nowrap gap-4 m-2">
                    ...
                    <div ref="imgContainer" class="flex-1 inline-flex justify-center items-center w-full h-full relative p-3 flex flex-col">
                        ...
                        <div :style="imgStyles" style="aspect-ratio: var(--ratio)" class="border-4 border-white bg-black/80 max-h-full max-w-auto flex flex-col">
                            <div v-show="selectedImageLoading" class="h-screen flex justify-center items-center">
                                <svg aria-hidden="true" class="w-16 h-16 text-white animate-spin fill-orange-400" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <Fade>
                            <div ref="infoMobile" class="flex sm:hidden bg-black z-[99] text-white font-bungee-hairline font-bold text-lg fixed p-1 bottom-0 left-0 w-full flex justify-between">
                                <div class="flex-1 flex flex-col">
                                    ...
                                    <p v-if="showInfo" class="flex-1 px-2 w-full text-xs text-white">{{selectedImage.description}}</p>
                                </div>
